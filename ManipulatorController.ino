#include <Undergrad_Thesis_Gesture_Recognition_inferencing.h>
...
/* Private variables ------------------------------------------------------- */
static const bool debug_nn = false; // Disable debug messages
...
void setup() {
    ...
    delay(5000); // Stabilization delay

    /* Connect used sensors */
    if (ei_connect_fusion_list(EI_CLASSIFIER_FUSION_AXES_STRING) == false) {
        return; // Exit if sensor list is invalid
    }
 ...
 */
void loop() {
    delay(2000); // Wait before sampling

    if (EI_CLASSIFIER_RAW_SAMPLES_PER_FRAME != fusion_ix) {
        return; // Exit if sensors don't match model requirements
    }
    ...
    // Turn the raw buffer into a signal
    signal_t signal;
    int err = numpy::signal_from_buffer(buffer, EI_CLASSIFIER_DSP_INPUT_FRAME_SIZE, &signal);
    if (err != 0) {
        return; // Exit if signal creation fails
    }
    ...
    // Determine the highest confidence gesture
    ...
    // Send gesture label via Serial
    if (max_confidence >= 0.99) {
        const char *gesture = result.classification[max_index].label;
        Serial.println(gesture); // Send gesture name to Python
        delay(1000); // Cooldown between detections
    }
}
